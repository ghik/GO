	...
	*pfout << "fillColor 0 0 0 1\n";
	...
	while (!points.empty()) {
		...
		while(sweep < x) {
			...
			*pfout << "line " << sweep << " 0 90\n";

			fout_paraboles(sweep);
			sweep += interv;
		}
	...
	for (arc* a = root; a != NULL; a = a->next) {
		...
		if (a->prev != NULL) {
			...
			if (a->s0 != NULL) {
				...
				*pfout << "segment " << a->s0->start.x << ' ' << a->s0->start.y
						<< ' ' << p0.x << ' ' << p0.y << endl;
			}
			y0 = clip(p0.y, Y0, Y1);
		}
		...
		*pfout << "lineColor 0 0 1 1\n";
		...
		if (d != 0) {
			double pa = -1 / (2 * d);
			*pfout << "parabola " << a->p.x + d / 2 << ' ' << a->p.y << ' '
					<< pa << ' ' << y0 << ' ' << y1 << endl;
		} else {
			*pfout << "segment " << a->p.x << ' ' << a->p.y << ' ' << p0.x
					<< ' ' << p0.y << endl;
		}
	}
}
...
bool circle(point a, point b, point c, double *x, point *o) {
	...
	double A = b.x - a.x, B = b.y - a.y, C = c.x - a.x, D = c.y - a.y, E = A
			* (a.x + b.x) + B * (a.y + b.y), F = C * (a.x + c.x)
			+ D * (a.y + c.y), G = 2 * (A * (c.y - b.y) - B * (c.x - b.x));
...
bool intersect(point p, arc *i, point *result) {
	...
	if ((!i->prev || a <= p.y) && (!i->next || p.y <= b)) {
		...
		result->x = (i->p.x * i->p.x
				+ (i->p.y - result->y) * (i->p.y - result->y) - p.x * p.x)
				/ (2 * i->p.x - 2 * p.x);
...
point intersection(point p0, point p1, double l) {
	...
	res.x = (p.x * p.x + (p.y - res.y) * (p.y - res.y) - l * l)
			/ (2 * p.x - 2 * l);
	return res;
}
